...
class EventFD
{
  public:
    ...
    DISALLOW_COPY_AND_MOVE_OPERATOR(EventFD);
...
class TaskScheduler
{
    ...
    class Timer
    {
      public:
        Timer();
        ~Timer();
        DISALLOW_COPY_AND_MOVE_OPERATOR(Timer);
  ...
  public:
    DISALLOW_COPY_AND_MOVE_OPERATOR(TaskScheduler);
  ...
  private:
    ...
    TimerID timerID_;
    std::map<TimerID, TimerPacket>
      timerTasks_; // 一个优化:对于repeat定时任务，下一任务时间点的计算方式。但我们的任务要求精度不高，无所谓。
